...
public class ScrollerNumberPicker extends View {
	...
	private class ItemObject {
		...
		public void drawSelf(Canvas canvas) {
			...
			if (isSelected()) {
				...
				float textSize = (float) normalFont
						+ ((float) (selectedFont - normalFont) * (1.0f - (float) moveToSelect
								/ (float) unitHeight));
				if (itemText.length() > 5 && itemText.length() <= 8) {
					textSize = 36.0f;
				}else if(itemText.length() > 8){
					textSize = 30.0f;
				}
			...
			/*StaticLayout layout = new StaticLayout(itemText, textPaint, 240,
					Alignment.ALIGN_NORMAL, 1.0F, 0.0F, true);

			// 从 (20,80)的位置开始绘制
			Log.e("jiang", (y + move + unitHeight / 2) + "");

			canvas.translate(0, y + move);
